(() => {
  ...
  function strainerCode(material,pumpFamily,suctionDn){const type=normalizePumpFamily(pumpFamily)==='BFI'||dnNumber(suctionDn)<=65?'THREAD_10':'FLANGE_16';return `${String(material||'GI').toUpperCase()}_${type}`}
  function strainerConnectionFor(pumpFamily,suctionDn){return strainerCode('GI',pumpFamily,suctionDn).includes('_THREAD_')?'Thread':'Flange'}
  ...
  function sourceBook(config){
    ...
    const accessoryBreakdown={suctionStrainer:null,suctionFlexible:null,dischargeFlexible:null},accessoryQty=Math.max(1,Math.min(6,Number(config.pumpQty)||1)),accessoryDn=String(config.suctionDn||'DN25'),strainerPriceCode=strainerCode(config.material,config.pumpFamily,accessoryDn),strainerConnection=strainerConnectionFor(config.pumpFamily,accessoryDn);
    function accessoryEntry(section){const priceCode=section==='strainer'?strainerPriceCode:code,row=accessoryPriceMyr(rowBy(section,accessoryDn),priceCode);return row?{...row,dn:accessoryDn,qty:accessoryQty,connection:section==='strainer'?strainerConnection:undefined,unitMyr:Number(row.baseMyr||0),totalMyr:Number(row.baseMyr||0)*accessoryQty}:null}
    ...
    return {book,parts,pumpDn,manifoldDn,autoManifoldDn,branchCode:code,strainerPriceCode,strainerConnection,header,headerVariant,tank,tankVariant,options:opts,accessoryBreakdown,accessoryAddonMyr,missing,basePriceable,priceable};
  }
  ...
  window.KeySuiteManifold={init,pageShown,renderProduct,renderPriceList,findConfiguredPrice,buildConfiguredItem,connectionForShutoffHead,connectionLabel,strainerCode,strainerConnectionFor,description,headerSizes,getGlobalOptions,setGlobalOptions,normalizeOptions};
})();
