...
async function keybotFastStockPriorityCandidates(service:any,candidates:any[]){
  ...
  const hot=evaluated.filter((x:any)=>x.keybot_price_available===true),pool=hot.length?hot:evaluated.filter((x:any)=>x.keybot_price_available!==true);
  return pool.sort(guidedSelectionCandidateCompare).slice(0,6);
}
...
async function keybotFastHandleProduct(service:any,telegramToken:string,companyId:string,chatId:string,senderId:string,session:any,user:any,customer:any,productText:any,allAssignedDuty=false){
  ...
  if(Number(parsed.flow_m3h)>0&&Number(parsed.head_m)>0&&(requestScope.recognized||allAssignedDuty)){
    ...
    const q=Number(parsed.flow_m3h),h=Number(parsed.head_m),stockPriority=allAssignedDuty||wanted.some((p:any)=>String(p?.price_group||'').toUpperCase()==='BFI');let candidates=await guidedSizeSelectedProducts(service,companyId,wanted,q,h,stockPriority?240:3,stockPriority?240:12);if(stockPriority)candidates=await keybotFastStockPriorityCandidates(service,candidates);if(!candidates.length){await telegramSend(telegramToken,chatId,'No suitable model was found for that Customer / Brand / Series / Duty.',mainMenuMarkup());return session}
    ...
    const chosen=candidates[0],product=chosen.product,pole=Number(chosen.pole||0),duty=dutyDisplay(String(productText||''),q,h).duty_text,pendingItem={...chosen,duty_text:duty},saved=await saveKeybotSession(service,companyId,chatId,senderId,{mode:'guided',step:'guided_product_action',flow_m3h:q,head_m:h,selected_customer_id:String(customer?.id||'')||null,context:{...sessionContext(session),keysuite_user_email:user.email,...(customer?{customer_name:customer.company_name}:{}),guided_product:product,pending_item:pendingItem,guided_duty_text:duty,guided_selection_candidates:candidates,guided_pending_request:{...parsed,pole,force_model:String(chosen.model||'')}}});await telegramSend(telegramToken,chatId,`${customer?`Customer: ${customer.company_name}\n\n`:''}Recommended - ${guidedSelectionPumpLabel(chosen)}\nMotor: ${oneDecimal(chosen.motor_kw||0)} kW${pole?` · ${pole}P`:''}\nDuty: ${duty}\n\nChoose action:`,guidedProductActionMenu(String(chosen.family||'').toUpperCase()==='ES'));return saved||session
  }
