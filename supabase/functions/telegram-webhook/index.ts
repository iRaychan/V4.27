...
async function guidedSizeSelectedProducts(service:any,companyId:string,products:any[],q:number,h:number,perProductLimit=3,totalLimit=12){
  ...
  for(const product of products||[]){
    ...
    for(const pole of poles){let rows:any[]=[];try{rows=selectPumpCandidates(guidedSelectorFamily(group),q,h,pole,Math.max(1,Math.min(12,Number(perProductLimit)||3)))||[]}catch(_){rows=[]}
...
async function keybotFastPricedCommonCandidates(service:any,candidates:any[]){
  const cache=new Map<string,any[]>(),priced:any[]=[];
  for(const candidate of candidates||[]){const product=candidate?.product,group=String(product?.price_group||'').toUpperCase();if(!['CHC_G1','CHC_G2','BFI','ES'].includes(group))continue;let rows=cache.get(group);if(!rows){try{rows=await guidedCatalogRows(service,product)}catch(_){rows=[]}cache.set(group,rows||[])}const keys=guidedUnique([candidate?.pricing_model,candidate?.model,candidate?.display_model].map((x:any)=>keybotFastPriceModelKey(group,x)).filter(Boolean)),row=(rows||[]).find((r:any)=>keys.includes(keybotFastPriceModelKey(group,r?.model)));if(!row)continue;const meta=keybotFastPriceRowMeta(group,row);if(meta.hasPrice&&meta.isCommon)priced.push({...candidate,keybot_price_available:true,keybot_price_rarity:'common'});}
  const closest=new Map<string,any>();for(const candidate of priced){const product=candidate?.product,key=[String(product?.key||product?.brand_id||product?.brand_name||''),String(product?.price_group||''),Number(candidate?.pole||0)].join('|'),current=closest.get(key);if(!current||Number(candidate?.selector_rank||999)<Number(current?.selector_rank||999)||(Number(candidate?.selector_rank||999)===Number(current?.selector_rank||999)&&guidedSelectionCandidateCompare(candidate,current)<0))closest.set(key,candidate)}
  return [...closest.values()].sort(guidedSelectionCandidateCompare).slice(0,6);
}
...
async function keybotFastHandleProduct(service:any,telegramToken:string,companyId:string,chatId:string,senderId:string,session:any,user:any,customer:any,productText:any,allAssignedDuty=false){
  ...
  if(Number(parsed.flow_m3h)>0&&Number(parsed.head_m)>0&&(requestScope.recognized||allAssignedDuty)){
    ...
    const q=Number(parsed.flow_m3h),h=Number(parsed.head_m);let candidates=await guidedSizeSelectedProducts(service,companyId,wanted,q,h,allAssignedDuty?6:3,allAssignedDuty?240:12);if(allAssignedDuty)candidates=await keybotFastPricedCommonCandidates(service,candidates);if(!candidates.length){await telegramSend(telegramToken,chatId,allAssignedDuty?'No suitable Common model with an input price was found for that duty.':'No suitable model was found for that Customer / Brand / Series / Duty.',mainMenuMarkup());return session}
    if(allAssignedDuty||family==='CHC'||(!family&&requestScope.brandKey)){const token=guidedSelectionNewToken(),duty=dutyDisplay(String(productText||''),q,h).duty_text,saved=await saveKeybotSession(service,companyId,chatId,senderId,{mode:'guided',step:'guided_selection_results',flow_m3h:q,head_m:h,selected_customer_id:String(customer?.id||'')||null,context:{...sessionContext(session),keysuite_user_email:user.email,...(customer?{customer_name:customer.company_name}:{}),guided_selection_products:wanted,guided_selection_keys:wanted.map((p:any)=>String(p.key)),guided_selection_candidates:candidates,guided_selection_token:token,guided_duty_text:duty,guided_pending_request:{...parsed,chc_scope:chcScope||'',flow_m3h:q,head_m:h}}});await telegramSend(telegramToken,chatId,`${customer?`Customer: ${customer.company_name}\n\n`:''}${guidedSelectionResultText(q,h,candidates,duty)}`,guidedSelectionResultKeyboard(candidates,token));return saved||session}
...
async function guidedSendExactRatedCurve(service:any,telegramToken:string,companyId:string,chatId:string,senderId:string,session:any,customer:any,product:any,exact:any,pole:any=0){
  ...
  const q=hasRequested?Number(requested.flow_m3h):Number(rated.flow_m3h),h=hasRequested?Number(requested.head_m):Number(rated.head_m),duty=hasRequested?String(requested.duty_text||`${oneDecimal(q)} m³/hr @ ${oneDecimal(h)} Mtr`):`${oneDecimal(q)} m³/hr @ ${oneDecimal(h)} Mtr`;
