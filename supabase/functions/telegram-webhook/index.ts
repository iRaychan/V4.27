...
async function guidedShowSelectionStockView(service:any,telegramToken:string,companyId:string,chatId:string,senderId:string,session:any,stockView:'hot'|'cold'){
  ...
  if(stockView==='cold'&&!cold.length){await telegramSend(telegramToken,chatId,'No suitable Cold Item model is available.',guidedSelectionResultKeyboard(hot,String(c.guided_selection_token||'legacy'),{stockView:'hot',hasCold:false,hasHot:hot.length>0}));return session}
  ...
  await telegramSend(telegramToken,chatId,`${prefix}${guidedSelectionResultText(Number(session.flow_m3h||0),Number(session.head_m||0),rows,String(c.guided_duty_text||''),stockView,cold.length>0)}`,guidedSelectionResultKeyboard(rows,String(c.guided_selection_token||'legacy'),{stockView,hasCold:cold.length>0,hasHot:hot.length>0}));return saved||session
}
...
async function guidedSizeSelectedProducts(service:any,companyId:string,products:any[],q:number,h:number,perProductLimit=3,totalLimit=12){
  const out:any[]=[];
...
async function keybotFastHandleProduct(service:any,telegramToken:string,companyId:string,chatId:string,senderId:string,session:any,user:any,customer:any,productText:any,allAssignedDuty=false){
  ...
  if(Number(parsed.flow_m3h)>0&&Number(parsed.head_m)>0&&(requestScope.recognized||allAssignedDuty)){
    const family=quoteFamilyFromText(raw),chcScope=quoteChcScopeFromText(raw),wanted=(products||[]).filter((p:any)=>p.has_curve===true).filter((p:any)=>allAssignedDuty||keybotFastProductInScope(p,requestScope)).filter((p:any)=>{const g=String(p.price_group||'').toUpperCase();if(allAssignedDuty||requestScope.series||(!family&&requestScope.brandKey))return true;if(family.startsWith('ES'))return g==='ES';if(family==='BFI')return g==='BFI';if(!['CHC_G1','CHC_G2'].includes(g))return false;return chcScope?g===chcScope:true;});
    const q=Number(parsed.flow_m3h),h=Number(parsed.head_m),hydraulic=await guidedSizeSelectedProducts(service,companyId,wanted,q,h,240,240);if(!hydraulic.length){await telegramSend(telegramToken,chatId,'No suitable model was found for that Customer / Brand / Series / Duty.',mainMenuMarkup());return session}
    const pools=await keybotFastStockCandidatePools(service,hydraulic),candidates=pools.hot,token=guidedSelectionNewToken(),duty=dutyDisplay(String(productText||''),q,h).duty_text,hasCold=pools.cold.length>0,saved=await saveKeybotSession(service,companyId,chatId,senderId,{mode:'guided',step:'guided_selection_results',flow_m3h:q,head_m:h,selected_customer_id:String(customer?.id||'')||null,context:{...sessionContext(session),keysuite_user_email:user.email,...(customer?{customer_name:customer.company_name}:{}),guided_selection_products:wanted,guided_selection_keys:wanted.map((p:any)=>String(p.key)),guided_selection_candidates:candidates,guided_selection_hot_candidates:pools.hot,guided_selection_cold_candidates:pools.cold,guided_selection_stock_view:'hot',guided_selection_token:token,guided_duty_text:duty,guided_pending_request:{...parsed,chc_scope:chcScope||'',flow_m3h:q,head_m:h}}});await telegramSend(telegramToken,chatId,`${customer?`Customer: ${customer.company_name}\n\n`:''}${guidedSelectionResultText(q,h,candidates,duty,'hot',hasCold)}`,guidedSelectionResultKeyboard(candidates,token,{stockView:'hot',hasCold,hasHot:pools.hot.length>0}));return saved||session
  }
...
async function manifoldSystemComponent(service:any,customerId:string,userEmail:string,pump:any,pumpQty:number,req:any){
  ...
  const material=bom.manifold_material==='SS'?'SS':'GI',code=`${material}_${conn.priceCode}`,pumpFamilyText=[pump?.family,pump?.keysuite_product_family,pump?.product_family,pump?.series,pump?.model,pump?.display_model].filter(Boolean).join(' ').toUpperCase(),isBfi=/\bBFI(?:N)?\b|\bHMS\b/.test(pumpFamilyText),strainerConnection=isBfi||dnNumberServer(pumpDn)<=65?'Thread':'Flange',strainerCode=`${material}_${strainerConnection==='Thread'?'THREAD_10':'FLANGE_16'}`,branch=by('branch',pumpDn),bv=findV(branch,code),header=by('header',`${material} ${headerDn}`),hv=findV(header,Math.max(1,Math.min(6,pumpQty)));const options={material,inlet_flexible:bom.inlet_flexible,outlet_flexible:bom.outlet_flexible,strainer:bom.strainer,strainer_connection:strainerConnection};let accessoryAddon=0;const missing:string[]=[];
  ...
  if(family==='CHC'){
    try{
      ...
      if(user){
        ...
        if(!hydraulic.length){await telegramSend(telegramToken,chatId,guidedSelectionResultText(q,h,[],display.duty_text),mainMenuMarkup());return session;}
        ...
        await telegramSend(telegramToken,chatId,guidedSelectionResultText(q,h,candidates,display.duty_text,'hot',hasCold),guidedSelectionResultKeyboard(candidates,token,{stockView:'hot',hasCold,hasHot:pools.hot.length>0}));return saved||session;
      }
...
.select('response_mode,keysuite_user_email')
    ...
    if(menuText||callbackData==='menu:home'){
      ...
      return json({ok:true,status:'keybot_menu',version:'V4.27.06'});
    }
    if(newRequestButton){
      session=await saveKeybotSession(service,keySuiteCompanyId,chatId,senderId,{mode:'',step:'idle',flow_m3h:null,head_m:null,flow_raw:null,head_raw:null,selected_customer_id:null,context:{}});
    ...
    if(!callbackQuery&&text&&!menuText){
      ...
      if(user&&keybotFastExactMayOverrideSession(session)&&keybotFastLooksLikePump(rawTelegramText)&&keybotFastLooksLikeExactPumpModel(rawTelegramText)){
        ...
        const products=await guidedUserAvailableProducts(service,keySuiteCompanyId,user),matches=keybotFastPrepareMatches(await keybotFastModelMatches(service,keySuiteCompanyId,products,rawTelegramText),rawTelegramText),requestedDuty=keybotFastRequestedDuty(rawTelegramText),fastContext={keysuite_user_email:user.email,...(requestedDuty?{keybot_fast_requested_duty:requestedDuty}:{})};if(matches.length===1){session=await keybotFastOpenPreparedMatch(service,telegramToken,keySuiteCompanyId,chatId,senderId,session,null,matches[0],fastContext);return json({ok:true,status:matches[0]?.keybot_fast_choose_pole?'fast_exact_es_pole':'fast_exact_model'})}if(matches.length>1){session=await saveKeybotSession(service,keySuiteCompanyId,chatId,senderId,{mode:'guided',step:'guided_fast_model_choice',selected_customer_id:null,context:{...fastContext,guided_fast_matches:matches}});await telegramSend(telegramToken,chatId,'More than one matching model was found. Choose the desired Brand / Model:',keybotFastMatchKeyboard(matches));return json({ok:true,status:'fast_model_choice',count:matches.length})}await telegramSend(telegramToken,chatId,`No exact assigned pump model matched “${rawTelegramText}”.\n\nTry the exact Brand + Model, for example B.G.Reich ES 80-26 4P.`,mainMenuMarkup());return json({ok:true,status:'fast_exact_model_not_found'});
      }
...
${error instanceof Error?error.message:String(error)}`,guidedSelectedCustomerMenu());return json({ok:true,status:'guided_customer_selection_error'})}
    ...
    if(session?.mode==='guided'&&session?.step==='guided_selection_confirm_duty'&&startSizingButton){
      const user=await linkedKeySuiteUser(service,keySuiteCompanyId,senderId),customerId=String(session.selected_customer_id||''),customer=customerId?await guidedAllowedCustomerById(service,keySuiteCompanyId,user,customerId):null,c=sessionContext(session),products=Array.isArray(c.guided_selection_products)?c.guided_selection_products:[],keys=new Set((Array.isArray(c.guided_selection_keys)?c.guided_selection_keys:[]).map((x:any)=>String(x))),chosen=products.filter((x:any)=>keys.has(String(x.key))),q=Number(session.flow_m3h||0),h=Number(session.head_m||0);if(!user||(customerId&&!customer)){await telegramSend(telegramToken,chatId,'The Selection session has expired. Start a New Request.',mainMenuMarkup());return json({ok:true,status:'guided_selection_expired'})}const hydraulic=await guidedSizeSelectedProducts(service,keySuiteCompanyId,chosen,q,h,240,240);if(!hydraulic.length){await telegramSend(telegramToken,chatId,guidedSelectionResultText(q,h,[],String(c.guided_duty_text||'')),telegramReplyKeyboard([['✏️ Change Duty','☑ Brand / Series'],['⬅️ Back','🔄 New Request']]));return json({ok:true,status:'guided_selection_no_result'})}const pools=await keybotFastStockCandidatePools(service,hydraulic),candidates=pools.hot,hasCold=pools.cold.length>0;session=await saveKeybotSession(service,keySuiteCompanyId,chatId,senderId,{mode:'guided',step:'guided_selection_results',flow_m3h:q,head_m:h,selected_customer_id:customerId||null,context:{...c,guided_selection_candidates:candidates,guided_selection_hot_candidates:pools.hot,guided_selection_cold_candidates:pools.cold,guided_selection_stock_view:'hot',guided_selection_token:guidedSelectionNewToken(),guided_pending_request:{...(c.guided_pending_request||{}),flow_m3h:q,head_m:h}}});const resultContext=sessionContext(session),resultToken=String(resultContext.guided_selection_token||'legacy');await telegramSend(telegramToken,chatId,guidedSelectionResultText(q,h,candidates,String(c.guided_duty_text||''),'hot',hasCold),guidedSelectionResultKeyboard(candidates,resultToken,{stockView:'hot',hasCold,hasHot:pools.hot.length>0}));return json({ok:true,status:'guided_selection_results',count:candidates.length,cold_count:pools.cold.length});
    }
