/* KeySuite V4.27.03 — Brand-level tick controls all visible series; CHC and BFI Enhanced are available but default unticked. Dashboard Brand / Series follows User Assigned when no customer, and User Assigned × Customer Price Preference when selected.
...
.ks42703-cold-inline{display:inline-flex;align-items:center;gap:7px;color:#17365d;font-size:12px;font-weight:700;line-height:1.2;cursor:pointer;white-space:nowrap}.ks42703-cold-inline input{width:auto!important;min-height:0!important;margin:0!important;accent-color:#1264a3}
...
function syncColdItemCheck(){state.includeColdItems=$('ks42703IncludeCold')?.checked===true}
...
let coldLabel=$('ks42703ColdLabel');if(!coldLabel){coldLabel=document.createElement('label');coldLabel.id='ks42703ColdLabel';coldLabel.className='ks42703-cold-inline';coldLabel.title='Include suitable pump models without a filled price';coldLabel.innerHTML='<input type="checkbox" id="ks42703IncludeCold"><span>Cold Item</span>';checkRow.appendChild(coldLabel)}const coldCheck=$('ks42703IncludeCold');if(coldCheck)coldCheck.checked=state.includeColdItems;
...
function bind(){const box=$('ksDashboardDutyFinder');if(!box||box.dataset.v39444Bound)return false;box.dataset.v39444Bound='1';
  ...
  const changed=ev=>{if(!ev.target.matches('#ksDashFlow,#ksDashHead,#ksDashFlowUnit,#ksDashHeadUnit,#ks42703IncludeCold,input[data-enhanced-key]'))return;if(ev.target.matches('input[data-enhanced-key]'))syncEnhancedChecks();if(ev.target.matches('#ks42703IncludeCold'))syncColdItemCheck();ev.stopPropagation();ev.stopImmediatePropagation();cancelPending();const st=$('ksDutyStatus');if(st)st.textContent='Press Check Pumps to update results.'};
...
window.KeySuiteV39442Dashboard={version:'4.27.03',runSelected,renderResults,renderPreference,loadPreference,masterSeries,seriesFor,brandSeriesFor,cancelPending,removeMaterialControls,presentationContext};
